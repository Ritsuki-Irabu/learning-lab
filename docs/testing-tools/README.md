...
| 2026-08-04 | テスト仕様書作成 | 単体テスト・結合テスト・総合テストでは見る範囲が違うため、それぞれの領域に絞って観点を設ける。範囲を混同すると確認項目が膨らみやすい |
| 2026-08-04 | 結合テスト設計 | 機能・画面単位で流れを整理し、DBや外部インターフェースとの入出力整合性を確認する。正常系だけでなく異常系、前提条件、必要エビデンスまで明確にする |
