...
テスト仕様書は、何をどの粒度で確認するかによって大きく分けられる。

| 分類 | 主な確認対象 | 観点 |
|---|---|---|
| 単体テスト | 関数・メソッド・クラスなどの小さな部品 | 個別ロジック、入力値ごとの分岐、境界値 |
| 結合テスト | 機能・画面・DB・外部インターフェースのつながり | 画面遷移、登録・更新時のDB反映、入出力の整合性 |
| 総合テスト | システム全体の業務フロー | ユーザー操作、業務シナリオ、複数機能をまたぐ流れ |

単体・結合・総合は、それぞれ見る範囲が違う。結合テストで単体レベルの細かいロジックまで追いすぎたり、総合テスト相当の業務フローまで広げすぎたりすると、観点が膨らんで時間を取られやすい。

### 結合テスト設計の考え方

結合テストは、機能単位・画面単位でシステムの流れを整理し、部品同士が正しくつながっているかを確認するテスト。

特に見るポイント：

| 観点 | 確認すること |
|---|---|
| 画面・機能の流れ | 入力 → 確認 → 登録 → 完了など、画面遷移や処理順が仕様通りか |
| DBとのつながり | 登録・更新・削除時に、対象テーブルへ正しい値が反映されるか |
| 入出力の整合性 | 画面入力、DB保存、API連携、帳票出力などの値がずれていないか |
| 正常系 | 期待通りの入力で、仕様通りに処理が完了するか |
| 異常系 | 必須漏れ、不正値、重複、権限不足、外部連携失敗などを想定できているか |
| 前提条件 | ログイン状態、事前データ、権限、マスタ設定などが明確か |
| エビデンス | 実行結果として何を残すか。画面キャプチャ、DB確認結果、ログ、レスポンスなど |

結合テストでは「データを入れたら画面に出るか」だけでなく、**どの入力が、どの処理を通り、どこに保存・出力されるか** を追う。正常系だけでなく異常系も必要になるため、設計書・仕様書を読んでシステム間の整合性を理解しておくことが前提になる。

### 結合テストと単体テストの線引き

結合テスト仕様書を作る前に、単体テストで見る範囲と結合テストで見る範囲を具体的に分けておく。ここが曖昧だと、結合テスト中に個別ロジックの細かい分岐まで追い始めて、テスト観点が膨らみやすい。

| 観点 | 単体テストで見る | 結合テストで見る |
|---|---|---|
| 対象範囲 | 関数、メソッド、クラス、単一コンポーネント | 画面、機能、Controller〜Model〜DB、API、外部IFのつながり |
| 主な目的 | 小さな部品が単独で正しく動くか | 部品を組み合わせたときに、入出力・状態・データが仕様通りつながるか |
| 入力値の見方 | 境界値、同値分割、分岐条件、例外条件を細かく見る | 画面・機能として意味のある入力パターンに絞る |
| DB確認 | 原則モックや限定的な検証。永続化ロジック単体を見る | 登録・更新・削除後に、対象テーブルへ正しい値が反映されるかを見る |
| 画面確認 | 画面全体の流れは基本見ない | 入力、遷移、表示、メッセージ、権限、状態変化を見る |
| 異常系 | 関数単位の不正値、例外、バリデーション分岐 | 画面・機能としてのエラー表示、DB未更新、外部連携失敗時の挙動 |
| エビデンス | テストコード結果、カバレッジ、ログ | 画面キャプチャ、DB確認結果、APIレスポンス、ログ、帳票など |

#### 線引きの判断基準

| 迷った観点 | 判断 |
|---|---|
| 関数の中のif分岐を1つずつ確認したい | 単体テスト寄り |
| 入力した値がDBに登録され、次画面に表示されるか見たい | 結合テスト |
| バリデーションのルール単体を網羅したい | 単体テスト寄り |
| バリデーションエラー時に画面へ戻り、入力値やエラーメッセージが正しく出るか見たい | 結合テスト |
| SQLのwhere条件だけを細かく確認したい | 単体テストまたはリポジトリ層のテスト |
| 画面操作の結果、想定したデータだけが抽出・更新されるか見たい | 結合テスト |
| 複数機能をまたいだ業務シナリオ全体を確認したい | 総合テスト寄り |

#### 仕様書作成前に決めること

- 今回の結合テスト対象は「どの画面・どの機能」か
- 単体テストで確認済みとして扱うロジックは何か
- 結合テストで確認するDBテーブル・カラムはどこか
- 外部API、ファイル出力、メール、帳票などのインターフェースを含むか
- 正常系・異常系それぞれで、画面・DB・ログのどこをエビデンスにするか
- 総合テストに回す業務フローはどこからどこまでか

テスト実行時のつまずきを減らすには、テストケースに以下を明記する。

- 前提条件（ログインユーザー、権限、事前データ、環境）
- 操作手順
- 入力値
- 期待結果
- DB・画面・外部インターフェースで確認する箇所
- 必要なエビデンス
...
| 2026-07-29 | テスト仕様書作成 | 単体テスト・結合テスト・総合テストでは見る範囲が違うため、それぞれの領域に絞って観点を設ける。範囲を混同すると確認項目が膨らみやすい |
| 2026-07-30 | 結合テスト設計 | 機能・画面単位で流れを整理し、DBや外部インターフェースとの入出力整合性を確認する。正常系だけでなく異常系、前提条件、必要エビデンスまで明確にする |
| 2026-08-05 | 結合テストと単体テストの線引き | 結合テスト仕様書では、単体テストで見る関数・分岐・境界値の細部まで追いすぎない。画面・機能単位で、入力からDB反映、表示、外部IFまでのつながりを確認する範囲に絞る。 |
